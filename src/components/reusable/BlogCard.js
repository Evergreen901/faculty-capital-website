...
const BlogCard = ({ imgUrl, type, caption, description, clickHandler, detail = false }) => {
    ...
    return (
        <div className={clsx('blog-card-container', detail ? 'detail' : '')} onClick={clickHandler}>
            ...
            <div
                className={clsx(
                    'bg-white bg-origin-padding bg-no-repeat overflow-hidden mask-area-vertical',
            ...
            >
                <span
                    className={clsx(
                        ...
                        type.includes(BLOG_TAG_TYPE.PODCAST) ? 'text-[#E55800]' : 'text-[#0000FF]'
                    )}
                >
                ...
                {!detail && (
                    <h4 className="font-bold text-[#303336] text-[22px] leading-[28px] -tracking-[0.55px] py-2 transition">
                        {caption}
                    </h4>
                )}
